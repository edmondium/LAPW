...
#include <cmath>  // std::sph_bessel, std::legendre (C++17)
#include <boost/math/special_functions/factorials.hpp>  // Boost factorials

// Computes the double factorial (l!!) using Boost
...
{
    return std::sph_bessel(l, x); // Uses built-in function
}
...
inline double Bessel_j(int l, double x)
{
    ...
    double j2 = 0, j1 = 1;
    double j0, jl, x1 = 1 / x; // Store 1/x for performance
...
{
    return std::legendre(l, x); // Uses the standard function in C++17+
}
